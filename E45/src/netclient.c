...
#include "../libraries/headers/ringbuffer.h"
...
int read_some(RingBuffer *buffer, int fd, int is_socket)
{
	...
	rc = (is_socket)
		? recv(fd, RingBuffer_starts_at(buffer), RingBuffer_available_space(buffer), 0)
		: read(fd, RingBuffer_starts_at(buffer), RingBuffer_available_space(buffer));
...
int write_some(RingBuffer *buffer, int fd, int is_socket)
{
	...
	rc = (is_socket)
		? send(fd, bdata(data), blength(data), 0)
		: write(fd, bdata(data), blength(data));
