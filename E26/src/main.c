...
int main(int argc, const char *argv[])
{
	...
	while (apr_getopt(opt, "I:Lc:m:i:d:SF:B:", &ch, &optarg) == APR_SUCCESS)
	{
		switch (ch)
		{
			...
			case 'L': request = COMMAND_LIST; break;
			case 'c': config_opts = optarg; break;
			case 'm': make_opts = optarg; break;
			case 'i': install_opts = optarg; break;
			case 'S': request = COMMAND_INIT; break;
			case 'F':
			{
	...
	switch (request)
	{
		...
		case COMMAND_LIST: DB_list(); break;
		case COMMAND_FETCH:
		{
		...
		default: sentinel("Invalid command given.");
	}
